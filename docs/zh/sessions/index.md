...
Agents SDK 提供内置的会话记忆，可在多次智能体运行之间自动维护对话历史，无需在回合间手动处理 `.to_input_list()`。

会话按特定会话存储对话历史，使智能体在无需显式手动内存管理的情况下保持上下文。这对构建聊天应用或需要记住先前交互的多轮对话尤为有用。
...
3. **上下文保持**：使用同一会话的后续运行会包含完整对话历史，使智能体能够保持上下文。
...
- 对临时对话使用内存型 SQLite（`SQLiteSession("session_id")`）
- 对持久对话使用文件型 SQLite（`SQLiteSession("session_id", "path/to/db.sqlite")`）
- 对使用 SQLAlchemy 支持的现有数据库的生产系统，使用基于 SQLAlchemy 的会话（`SQLAlchemySession("session_id", engine=engine, create_tables=True)`)
- 面向云原生生产部署，使用 Dapr 状态存储会话（`DaprSession.from_address("session_id", state_store_name="statestore", dapr_address="localhost:50001")`），支持 30+ 数据库后端，并内置遥测、追踪与数据隔离
...
- [`AdvancedSQLiteSession`][agents.extensions.memory.advanced_sqlite_session.AdvancedSQLiteSession] - 具备分支与分析能力的增强版 SQLite
